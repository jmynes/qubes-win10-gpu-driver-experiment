...
// --- Qubes diagnostic logging (temporary, M1 monitor-enumeration debug) ---
// The IddCx UMDF host runs as LOCAL SERVICE with a write-restricted token, so it
// cannot write arbitrary files. Write diagnostics to the device's own registry key
// (HKLM\...\Enum\<dev>\Device Parameters\DiagNN) which the host CAN write.
#include <stdio.h>
#include <stdarg.h>
static WDFDEVICE g_LogDevice = nullptr;
static volatile LONG g_LogSeq = 0;
static void QLog(const char* fmt, ...)
{
    ...
    if (!g_LogDevice) return;
    WDFKEY key;
    if (NT_SUCCESS(WdfDeviceOpenRegistryKey(g_LogDevice, PLUGPLAY_REGKEY_DEVICE, KEY_WRITE, WDF_NO_OBJECT_ATTRIBUTES, &key)))
    {
        WCHAR wbuf[480]; MultiByteToWideChar(CP_ACP, 0, buf, -1, wbuf, _countof(wbuf));
        WCHAR vname[24]; swprintf_s(vname, _countof(vname), L"Diag%02ld", InterlockedIncrement(&g_LogSeq));
        UNICODE_STRING vn; RtlInitUnicodeString(&vn, vname);
        UNICODE_STRING val; RtlInitUnicodeString(&val, wbuf);
        WdfRegistryAssignUnicodeString(key, &vn, &val);
...
void IndirectDeviceContext::InitAdapter()
{
    ...
    QLog("InitAdapter: IddCxAdapterInitAsync=0x%08X", Status);
...
void IndirectDeviceContext::FinishInit(UINT ConnectorIndex)
{
    ...
    QLog("FinishInit(%u): IddCxMonitorCreate=0x%08X", ConnectorIndex, Status);
    if (NT_SUCCESS(Status))
    {
        ...
        QLog("FinishInit(%u): IddCxMonitorArrival=0x%08X", ConnectorIndex, Status);
    }
}
...
NTSTATUS IddSampleAdapterInitFinished(IDDCX_ADAPTER AdapterObject, const IDARG_IN_ADAPTER_INIT_FINISHED* pInArgs)
{
    ...
    QLog("AdapterInitFinished: AdapterInitStatus=0x%08X", pInArgs->AdapterInitStatus);
